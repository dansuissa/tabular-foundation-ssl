...
- `src/models/tfm_adapters.py` and related TFM modules pin checkpoint identity,
  normalize class order, and expose cold/warm inference metadata.
...
cell. For cluster runs, `src/run_single.py` writes a temporary JSON shard and
atomically renames it after serialization. This prevents preemption from leaving
a half-written success record.

The cluster scripts separate concerns:

| Component | Responsibility |
|---|---|
| `preflight.py` | Validate packages, checkpoints, configuration, and resources. |
| `submit_wave.py` | Materialize the full task map and Slurm array script. |
| `monitor_wave.py` | Report completed, failed, corrupt, and missing shards. |
| `collect_wave.py` | Validate key uniqueness and produce a combined CSV. |

Report builders consume canonical CSVs, not scheduler output. Every figure has
a machine-readable table upstream, and each final report directory contains a
manifest and integrity audit.
...
6. Run a one-cell smoke, then a representative failure gate, before a full wave.
7. Preserve the task map, source/config hashes, package and checkpoint identity,
