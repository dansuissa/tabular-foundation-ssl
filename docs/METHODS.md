...
This document defines the methods behind the canonical result claims. It also
separates evaluated methods from exploratory code that remains available for
future work. The authoritative machine-readable registry is
...
- **Transductive experiments:** are explicitly named and excluded from all
  canonical rankings.
...
An internal stratified labeled holdout controls early stopping when possible.
...
## Exploratory and non-canonical code

The repository retains one-shot TFM pseudo-labeling, CAST-style selection,
teacher/student and consensus variants, prior adjustment, TFM geometric
adapters, STUNT/SeBA/D2R2 adapters, component ablations, and separately named
transductive experiments. They are useful scaffolding for future work but do
not have the complete validated 10 × 4 × 3 canonical grid, so they are not
included in the 25-method result table.

`tabpfn3_distpfn_transductive` is an explicit unsupported placeholder. The code
refuses to substitute a heuristic for a faithful DistPFN implementation. See
[`tfm_ssl_fidelity.md`](tfm_ssl_fidelity.md) for the complete deviation ledger.

Primary papers and official model resources are collected in
[`REFERENCES.md`](REFERENCES.md). A citation there identifies the origin of a
method family; it does not override the implementation-fidelity qualifications
in this document.
