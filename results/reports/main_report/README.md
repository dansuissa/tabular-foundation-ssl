...
The full Phase-A metric table is `tables/focused_ssl/new_method_metric_summary.csv`; the exact
...
`tables/focused_ssl/representative_attention_final_gate.csv`; obsolete pre-fix collapse
experiments are retained under `results/validation/` for failure analysis.

## Calibration and secondary metrics

`tables/focused_ssl/new_method_metric_summary.csv` reports all requested metrics with valid
...
focused SSL phases. PNG is the single canonical figure format in this report.
...
`tables/focused_ssl/requested_methods_and_historical_baselines.csv` compares all eight new
...
summary are retained in [`tables/historical/`](tables/historical/). Lower-level
wave outputs remain under `results/aggregated/`; they are evidence, not separate
reports.
...
The final cluster test command completed with **25 passed and 1 skipped**. After
repository packaging added a source-level leakage guard, the current repository
suite contains **26 passing tests and 1 optional skip**. Captured records are in
`results/validation/pytest_current.txt` and
`results/validation/github_repository_pytest.txt`.
...
4. The original cluster execution directory was not a Git work tree, so the run
   payloads use deterministic source/configuration hashes. The packaged research
   record is now versioned in Git without rewriting those historical identities.
...
- Reproducibility manifest: `validation/run_manifest.json`
- Integrity audit: `validation/integrity_validation.json`
