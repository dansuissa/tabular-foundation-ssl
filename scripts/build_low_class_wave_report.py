"""Build polished supervisor-facing analysis for the low-class wave.

Read-only w.r.t. results/raw and results/aggregated. Writes ONLY into
results/reports/low_class_wave_paper_methods/ (figures + machine-readable summary
+ a stats dump used to author the markdown report).
...
OUT = Path("results/reports/low_class_wave_paper_methods")
FIG = OUT / "figures"
...
def main():
    ...
    ds_table.to_csv(OUT / "dataset_table.csv", index=False)
    ...
    overall.to_csv(OUT / "overall_method_ranking.csv", index=False)
    ...
    best_out.to_csv(OUT / "best_method_by_dataset_budget.csv", index=False)
    ...
    # ---------------- write JSON ----------------
    with open(OUT / "report_summary.json", "w") as f:
    ...
    print("\nDONE. Figures + summary written to", OUT)
